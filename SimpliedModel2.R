...
init5 <- list(
  log10_theta = c(log10(1.5), log10(8e-6),
                  log10(20), log10(1e-5), 
                  log10(1.5), log10(1)),
  theta = c(3,20,3.3e-3)
  sigma = c(1, 1))
...
Estimate_Full_06_OCT <- stan("New_model_Stan.stan",
...
posterior_sample_table = data.frame(epsilon1 = posterior_samples_merged_after_burnin$theta_KO[,1],
                                    ...
                                    epsilon2 = posterior_samples_merged_after_burnin$theta_KO[,7],
                                    #k2 = posterior_samples_merged_after_burnin$theta[,9],
                                    delta_M = posterior_samples_merged_after_burnin$theta[,8],
                                    phi = posterior_samples_merged_after_burnin$theta_KO[,9])
...
ggplot(posterior_sample_table, aes(x = log10(epsilon1))) + 
  geom_histogram(breaks=seq(-1,2,2/50),aes(y = ..density.., color = 'posterior')) + 
  stat_function(fun = function(x) {dnorm(x, 0, 2)}, aes(color = 'prior')) +
  geom_vline(aes(xintercept=log10(quantile(epsilon1, 0.025))),
             linetype="dashed",color = 'red') + 
  geom_vline(aes(xintercept=log10(quantile(epsilon1, 0.5))),
...
ggplot(posterior_sample_table, aes(x = delta_I)) + 
  geom_histogram(breaks=seq(0,50,0.05),aes(y = ..density.., color = 'posterior')) + 
...
ggplot(posterior_sample_table, aes(x = log10(p))) + 
  geom_histogram(breaks=seq(-6,6,12/100),aes(y = ..density.., color = 'posterior')) + 
...
ggplot(posterior_sample_table, aes(x = delta_V)) + 
...
ggplot(posterior_sample_table, aes(x = log10(epsilon2))) + 
  ...
  geom_vline(aes(xintercept=log10(quantile(epsilon2, 0.025))),
             linetype="dashed",color = 'red') + 
  geom_vline(aes(xintercept=log10(quantile(epsilon2,0.5))),
             linetype="solid",color = 'red') + 
...
Within_host_model = function(t, y, theta){
  ...
  dydt4 =  330 + theta[7] * theta[9] * y[3] - theta[8] * y[4];
...
for (i in 1:n_iteration){
  y_init = c(data_combined_muc1_V_W$T0, data_combined_muc1_V_W$I0, data_combined_muc1_V_W$V0, 330/posterior_sample_table$delta_M[i])
  param_fit = c(1,
...
write.csv(posterior_samples_merged_after_burnin, file="Muc1_fit_NOV_4.csv")
