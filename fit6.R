...
Estimate_Full_26_Sep <- stan("PriorStan.stan",
                             data = data_combined_muc1_V_W,
                             pars = c("theta","sigma"),
                             seed = 20200926,  # set random seed for reproducibility
                             iter = 10000,
                             chains = 2,
...
print(Estimate_Full_26_Sep, pars = c("theta","sigma"))
stan_dens(Estimate_Full_26_Sep, pars = c("theta"), separate_chains = TRUE,nrow = 6)


# extract posterior samples for selected parameters
posterior_samples_all = rstan::extract(Estimate_Full_26_Sep, pars = c("theta"), inc_warmup = TRUE, permuted = FALSE)
posterior_samples_merged_after_burnin = rstan::extract(Estimate_Full_26_Sep, pars = c("theta"))
...
posterior_sample_table = data.frame(epsilon1 = posterior_samples_merged_after_burnin$theta[,1],
                                    ...
                                    delta_V = posterior_samples_merged_after_burnin$theta[,6],
                                    kappa_M = posterior_samples_merged_after_burnin$theta[,7],
                                    q_V = posterior_samples_merged_after_burnin$theta[,8],
                                    s = posterior_samples_merged_after_burnin$theta[,9],
                                    eta = posterior_samples_merged_after_burnin$theta[,10],
                                    delta_M = posterior_samples_merged_after_burnin$theta[,11],
                                    phi = posterior_samples_merged_after_burnin$theta[,12],
                                    epsilon2 = posterior_samples_merged_after_burnin$theta[,13],
                                    k2 = posterior_samples_merged_after_burnin$theta[,14],
                                    V0 = posterior_samples_merged_after_burnin$theta[,15])
...
for (i in 1:1000){
  y_init = c(data_combined_muc1_V_W$T0, data_combined_muc1_V_W$I0, posterior_sample_table$V0[i], posterior_sample_table$s[i]/posterior_sample_table$delta_M[i], data_combined_muc1_V_W$MA0, data_combined_muc1_V_W$MP0)
  ...
  M_ppc_WT[i,] = model_output_WT[,5] + model_output_WT[,6] + model_output_WT[,7]
  
  V_ppc_KO[i,] = model_output_KO[,4]
  M_ppc_KO[i,] = model_output_KO[,5] +  model_output_KO[,6] +  model_output_KO[,7] 
}
...
##============================================================================================##
posterior_samples_merged_after_burnin = read_excel('Muc1_fit_full_fit4(13)_Mr.xls')
...
for (i in 1:1000){
  y_init = c(data_combined_muc1_V_W$T0, data_combined_muc1_V_W$I0, data_combined_muc1_V_W$V0, 2e+4, 0, data_combined_muc1_V_W$MP0)
  param_fit = c(posterior_sample_table$epsilon1[i],
                posterior_sample_table$k1[i],
                posterior_sample_table$beta[i], 
                posterior_sample_table$delta_I[i], 
  ...
  model_output_WT = ode(times = t_ppc, y = y_init, func = Within_host_model2, parms = param_fit, method = "bdf")
  model_output_KO = ode(times = t_ppc, y = y_init, func = Within_host_model2, parms = param_fit_hat, method = "bdf")
