functions{
  real[]  Muc1_func(real t, real[] y, real[] theta, real[] x_r, int[] x_i){
    ...
    dydt[3] = theta[5] * y[2] - theta[6] * y[3] - theta[7] * y[3] * (y[4] + (theta[8]*y[3]*theta[9])/((theta[10] + theta[11] + theta[8]*y[3])*theta[11]));
    dydt[4] = theta[12] * (1 - theta[13] * t^10/(t^10 + theta[14]^10)) * (1 - y[4]/1.8e+5) * ((theta[8]*y[3]*theta[9])/((theta[10] + theta[11] + theta[8]*y[3])*theta[11]))  - theta[11] * y[4];
...
parameters{
  real<lower=0> theta[15]; // theta[1:15] = c(epsilon1, k1, beta, delta_I, p, delta_V, kappa_M, q_V, s, eta, delta_M, phi, epsilon2, k2, V0)
...
transformed parameters{
  ...
  real P[2]; // store values of s and delta_M
  
  P[1] = theta[9]; // s
  P[2] = theta[11]; // delta_M
  ...
  Macrophage_pred_WT[1:5] = rep_vector(y_hat_WT_Macrophage[1,4]+P[1]/P[2],5);
  Macrophage_pred_WT[6:10] = rep_vector(y_hat_WT_Macrophage[2,4]+P[1]/P[2],5);
  Macrophage_pred_WT[11:15] = rep_vector(y_hat_WT_Macrophage[3,4]+P[1]/P[2],5);
  Macrophage_pred_WT[16:19] = rep_vector(y_hat_WT_Macrophage[4,4]+P[1]/P[2],4);
  
  
  y_hat_KO_Macrophage = integrate_ode_bdf(Muc1_func, Y0, t0, time_data_Macrophage_KO, theta_hat, x_r, x_i);
  Macrophage_pred_KO[1:5] = rep_vector(y_hat_KO_Macrophage[1,4]+P[1]/P[2],5);
  Macrophage_pred_KO[6:9] = rep_vector(y_hat_KO_Macrophage[2,4]+P[1]/P[2],4);
  Macrophage_pred_KO[10:14] = rep_vector(y_hat_KO_Macrophage[3,4]+P[1]/P[2],5);
  Macrophage_pred_KO[15:20] = rep_vector(y_hat_KO_Macrophage[4,4]+P[1]/P[2],6);
...
theta[13] ~ normal(0.5,0.3); // epsilon2
theta[14] ~ normal(0.5,1); // k2
theta[15] ~ lognormal(log(3.5e-1),1); // V0
sigma[1] ~ normal(0,1);
sigma[2] ~ normal(0,1);
